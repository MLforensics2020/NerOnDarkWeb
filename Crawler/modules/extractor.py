...
def cinex(inputFile, outpath):
	...
	for line in f:
		...
		try:
			domainInfo = tldextract.extract(line)
			clpagename = domainInfo.domain+".htm"
			if len(clpagename) == 0:
			...
			print("Error: %s" % e)
			continue
		print("Good yet",outputFile)
		print(os.getcwd() + "/" + outpath + "/" + outputFile)
		# Extract page to file
		try:
			f = open(os.getcwd() + "/" + outpath + "/" + outputFile, 'w+')
			f.write(urllib.request.urlopen("http://google.com").read())
...
def extractor(website, crawl, outputFile, inputFile, outpath):
	...
	if len(inputFile) > 0:
		if crawl:
			print("while writing html, using cinex")
